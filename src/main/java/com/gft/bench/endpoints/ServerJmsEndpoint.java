...
public class ServerJmsEndpoint extends JmsEndpoint {
    ...
    public void sendEvent(ChatEvent event) {
        ...
        if (event.getType() == EventType.ENTER_ROOM) {
            try {
                Destination destination = session.createQueue("EVENT.QUEUE.TO.CLIENT");
                MessageProducer producer = session.createProducer(destination);
                TextMessage textMsg = session.createTextMessage(((EnterToRoomEvent) event).getRoom());
                textMsg.setBooleanProperty("client_entered_to_room", true);
    ...
    public void listenForEvent() { //ChatEventListener listener) {
        try {
            Destination serverEventQueue = session.createQueue("EVENT.QUEUE.TO.SERVER");
    ...
    public void onMessage(Message message) {
        try {
            if (message.getBooleanProperty("enter_to_room")){
